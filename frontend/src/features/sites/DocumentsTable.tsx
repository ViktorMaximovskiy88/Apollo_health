...
import { documentTableState, setDocumentTableFilter, setDocumentTableSort } from '../../app/uiSlice';
...
const columns = [
    ...
    },
    {
      ...
      render: ({ value: last_seen }: { value: string }) => {
        return prettyDateFromISO(last_seen);
      },
    },
    {
      header: 'Name',
    ...
    },
    {
      ...
      render: ({ value: doc_type_confidence }: { value?: number }) => {
        return (
          <>
            {doc_type_confidence &&
              `${Math.round(100 * doc_type_confidence)}%`}
          </>
        );
      },
    },
...
export function DocumentsTable() {
  ...
  const onFilterChange = useCallback((filter: any) => dispatch(setDocumentTableFilter(filter)), [dispatch]);
