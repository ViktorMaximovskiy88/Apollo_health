...
class Forward:
    @iterative_migration()
    async def url_to_urls(self, input_document: SingleUrlSite, output_document: NoScrapeConfigSite):
        if input_document.base_url is None:
            return
...
class Backward:
    @iterative_migration()
    async def urls_to_url(self, input_document: NoScrapeConfigSite, output_document: SingleUrlSite):
