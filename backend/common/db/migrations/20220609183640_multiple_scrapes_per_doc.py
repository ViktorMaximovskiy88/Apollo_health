...
class Forward:
    ...
    async def add_doc_ids(
        self, input_document: NoDocIdScrapeTask, output_document: SiteScrapeTask
    ):
        ret_docs = await RetrievedDocument.find(RetrievedDocument.scrape_task_id == input_document.id).to_list()
