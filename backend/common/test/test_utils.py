...
def mock_s3_client(monkeypatch: pytest.MonkeyPatch):
    def write_object(self, relative_key, temp_object_path) -> None:
        assert type(relative_key) == str
        assert type(temp_object_path) == str
        return None

    def write_object_mem(self, relative_key, object) -> None:
        assert type(relative_key) == str
    ...
    monkeypatch.setattr(
        BaseS3Client, "read_object_to_tempfile", read_object_to_tempfile
    )
