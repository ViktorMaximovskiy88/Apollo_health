...
def test_get_date_and_label():
    ...
    text = "Multiple Formats, left and right, 05/30 expire, published 02/21, updated 10/9/11, next review March 8, 2036, next update 10 January 2038"
    parser = DateParser(text, date_rgxs)
    parser.extract_dates()
...
def test_get_date_and_label_multiple_lines():
    ...
    new_parser = DateParser(text, date_rgxs)
    ...
    many_line_parser = DateParser(text, date_rgxs)
